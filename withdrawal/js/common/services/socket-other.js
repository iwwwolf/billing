(function () {
    ...
    app.service('socketRequest', function ($q, $rootScope, $timeout, eventService) {
            var requestById = {};
            var _url = '/echo/websocket';
            var protocol = location.protocol === 'http:' ? 'ws:' : 'wss:';
            var reconnectDelay = 500;
            var socketAuthSent = false;
            var socket;
            var connectPromise;

            function connect() {
                socket = new WebSocket(protocol + '//' + location.hostname.match(/billing.(.*)/)[1] + _url);

                connectPromise = $q(function (resolve, reject) {
                    var socketOpenTime;

                    var ssid = getCookie('ssid');

                    socket.onopen = function () {
                        socketOpenTime = new Date().getTime();
                    ...
                    socket.onmessage = function (e) {
                        ...
                        if (body.name === 'profile'){
                            if (body.msg) {
                                if(!socketAuthSent) {
                                    eventService.identifyUser(body.msg.user_id, $.cookie('platform'));
                                    eventService.send('system', 'billing_api-call', 1, {api_name: 'socket.get-profile'}, (new Date().getTime() - socketOpenTime));
                                    socketAuthSent = true;
                                }
                                resolve(socket);
                            } else {
                                if(!socketAuthSent) {
                                    eventService.send('system', 'billing_api-call', 0, {api_name: 'socket.get-profile'}, (new Date().getTime() - socketOpenTime));
                                    socketAuthSent = true;
                                }
                                $timeout (function() {
                                    socket.send(JSON.stringify({
            ...
            function response(data) {
                ...
                var requestPromise = requestById[body.request_id];
                if (!requestPromise) return;
                body.msg.timeDiff = new Date().getTime() - requestPromise.created;

                requestPromise.resolve(body.msg);

                delete requestById[body.requestId];
            }
            ...
            function request(action, payload) {
                ...
                var requestPromise = $q(function (resolve, reject) {
                    requestById[requestId].resolve = resolve;
                    requestById[requestId].reject = reject;
                    requestById[requestId].created = new Date().getTime();
                    requestById[requestId].name = action;
                });

                var timeoutPromise = $q(function (resolve, reject) {
                    setTimeout(function () {
                        requestById[requestId].timeDiff = new Date().getTime() - requestById[requestId].created;
                        reject(requestById[requestId]);
                        delete requestById[requestId];
                    }, 10000);
                });

                return $q.race([requestPromise, timeoutPromise]);
            }

           // connect();

            this.request = function (action, payload) {
